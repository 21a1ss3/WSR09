...
for($i=1; $i -le $usrCount; $i++)
{
    ...
    #(1):
    #Write-Host "SET PASSWORD FOR $cUsername = '$cPwd';"
    ...
    #(2):
    Write-Host "DROP USER IF EXISTS $cUsername;"    #ONLY IF YOU NEED RECREATE ACCOUNT / ТОЛЬКО ДЛЯ ПЕРЕСОЗДАНИЯ УЧЕТНОЙ ЗАПИСИ
    Write-Host "CREATE USER '$cUsername'@'%' IDENTIFIED BY '$cPwd';"

    #Recreating user database
    ...
    #Revoking privileges for preventing access a user to others user's databases in this server
    #Than granting privileges to specific database 
    #And saving privileges
    ...
    Write-Host "REVOKE ALL PRIVILEGES, GRANT OPTION FROM $cUsername;"
    Write-Host "GRANT ALL PRIVILEGES ON `$cUsername`.* TO $cUsername;"
